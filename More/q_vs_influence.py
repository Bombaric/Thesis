...
for i, n in enumerate(n_values):
    ...
    inf_vals.append(1.0 / (emp_inf+1.0))
    q_vals.append(q)
    inv_q_vals.append(q)
    variances.append(1.0-0.25 * variance)
    various.append(1.0/(variance+1.0))
...
#plt.plot(alphas, inf_vals, 'o-', color=color_inf, markersize=4, label=r'$1/(p+1)$')
...
#plt.plot(alphas, various, ':', color=color_var, linewidth=2, label=r'$var(\nu)$')
#plt.yscale('log')
plt.xlabel(r'Sample Complexity $\alpha = n/d$')
plt.ylabel('Value')
plt.title(r'q vs $1 - 1/4 Var(\nu)$ ($\sigma^2=0.01$)')
plt.legend()
plt.grid(True, which="both", linestyle='--', alpha=0.4)


plt.tight_layout()
plt.savefig('/home/ceci/Thesis/Plots/variance_nu_vs_q_nonlog.png', dpi=300)
