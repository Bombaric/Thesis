...
for i, lam in enumerate(reg_list):
    ...
    for n in n_values:
        ...
        logdet_sigma = np.linalg.slogdet((1+lam) * np.eye(d))[1] # logdet di (1+lambda)*I è d*log(1+lambda)
        # Fixed the minus sign to a plus sign: (1+lam)**(-1)
...
# --- Creazione GIF ---
with imageio.get_writer('/home/ceci/Thesis/Plots/influence_evolution_shifted.gif', mode='I', duration=0.5) as writer:
